...
import { Plus, FolderOpen, Trash2, Settings, Edit3, Save, X } from 'lucide-react'
...
export function ProjectsPage() {
  ...
  const { projects, currentProject, loadProjects, createProject, deleteProject, updateProject, setCurrentProject, restoreCurrentProject } = useProjectsStore()
        ...
        {projects.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {projects.map((project) => {
              ...
              const isDeleting = deleteConfirm.id === project.id

              return (
                <div
                  key={project.id}
                  onClick={() => handleOpenProject(project)}
                  className="group bg-white dark:bg-gray-800 rounded-xl shadow-sm hover:shadow-md transition-all duration-200 cursor-pointer border border-gray-200 dark:border-gray-700"
                >
                  <div className="p-4">
                    <div className="flex items-start justify-between gap-2 mb-2">
                      {isEditing ? (
                        <input
                          ...
                          className="flex-1 px-3 py-1.5 bg-gray-100 dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-lg text-gray-900 dark:text-white font-semibold text-base focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none"
                          autoFocus
                        />
                      ) : (
                    ...
                    {isEditing ? (
                      <textarea
                        ...
                        onClick={(e) => e.stopPropagation()}
                        rows={3}
                        className="w-full px-3 py-2 bg-gray-100 dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-lg text-gray-700 dark:text-gray-300 text-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none resize-none"
                        placeholder="添加项目描述..."
                      />
                    ) : (
