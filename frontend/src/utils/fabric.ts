...
export function createFabricDomain(domain: Domain, isEditable: boolean = false): any {
  ...
  const domainRect = new Rect({
    ...
    stroke: domain.borderColor,
    strokeWidth: domain.borderWidth,
  ...
  if (domain.titleVisible && domain.name) {
    ...
    const titleText = new Text(domain.name, {
      ...
      fontWeight: 'bold',
      fill: domain.titleColor || domain.borderColor,
      selectable: false,
      evented: false,
      left: 0,
      top: -20,
...
export function fabricObjectToNode(obj: any): Node | null {
  ...
  return {
    ...
    borderColor: obj.stroke || NODE_DEFAULTS.BORDER_COLOR,
    borderWidth: obj.strokeWidth || NODE_DEFAULTS.BORDER_WIDTH,
    borderRadius: obj.rx || NODE_DEFAULTS.BORDER_RADIUS,
