...
export function Header() {
  ...
  const { sidebarOpen, nodePoolOpen, aiSidebarOpen, toggleSidebar, setNodePoolOpen, setAiSidebarOpen, addErrorToast } = useUIStore()
  const { currentProject } = useProjectsStore()
  const { setCanvasId } = useCanvasStore()

  // 处理节点池开关 - 确保同侧只有一个开启
  const handleToggleNodePool = () => {
    const newState = !nodePoolOpen
    setNodePoolOpen(newState)
    if (newState) {
      setAiSidebarOpen(false)
    }
  }

  // 处理AI侧边栏开关 - 确保同侧只有一个开启
  const handleToggleAiSidebar = () => {
    const newState = !aiSidebarOpen
    setAiSidebarOpen(newState)
    if (newState) {
      setNodePoolOpen(false)
    }
  }
    ...
    <header className="h-14 border-b border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 flex items-center justify-between px-4">
      ...
      <div className="flex items-center gap-2">
        ...
        <button
          ...
          className={`p-2 rounded-lg transition-colors ${aiSidebarOpen
